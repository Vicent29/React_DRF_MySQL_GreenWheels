        ...
        <>
            <div className="dropstart">
                ...
                <ul className="dropdown-menu dropdown-menu-end dropdown-menu-lg-start bg-dark overflow-auto max-h-[20vh]" aria-labelledby="dropdownMenuButton1">
                    {notis.length != 0 ?
                        notis.map((noti, id) => {
                            return (<li key={id}><a className="dropdown-item  text-white bg-dark">{noti.desc[0].toUpperCase()}{noti.desc.substr(1, noti.desc.length - 1)}</a><hr className='m-1 sticky left-[2.5%] w-[95%] text-white' /></li>)
                        })
                        :
